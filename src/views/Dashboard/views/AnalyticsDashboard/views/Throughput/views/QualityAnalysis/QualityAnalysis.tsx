...
const ThroughputQualityAnalysis = ({ data }: PageProps) => {
  let colourIdx = 0;

  const mySeries = data.map((item) => {
    let colour = '';

    if (item.type === 'Failure Demand') {
      colour = '#E1523E';
    } else if (item.type === 'Value Demand') {
      colour = '#2AD2C9';
    } else if (item.type === 'Non-value Demand') {
      colour = '#F5B24B';
    } else {
...
export default ThroughputQualityAnalysis;
