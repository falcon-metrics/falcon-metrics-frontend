...
interface PageProps {
  data: Array<{ areaName: string; count: number }>;
}

const ThroughputValueAreaAnalysis = ({ data }: PageProps) => {
  let colourIdx = 0;

  const mySeries = data.map((item) => {
    let colour = '';

    if (item.areaName === 'Not classified') {
      colour = '#F5B24B';
    } else {
...
export default ThroughputValueAreaAnalysis;
