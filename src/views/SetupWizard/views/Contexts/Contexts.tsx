...
import { Tree, Option, RouteParams } from "./Contexts.data";
...
const ContextsPage = ({ initialValues = [], options, submit }: Props) => {
  ...
  const resolver: any = yupResolver(validationSchema(isAzure()));
    ...
    async ({ contexts }: FormValues) => {
      ...
      const data = dataset.map((value) => {
        if (value.address) {
          const option = find(options, {
            id: Array.isArray(value.address) ? value.address[0] : value.address,
          });
          value.projectId =
      ...
      try {
        ...
        if (initialValues.length) {
          enqueueSnackbar(
            "We are setting up your dashboard to reflect the changes. This may take a few minutes.",
            { variant: "warning" }
          );
        }
      } catch (err) {
