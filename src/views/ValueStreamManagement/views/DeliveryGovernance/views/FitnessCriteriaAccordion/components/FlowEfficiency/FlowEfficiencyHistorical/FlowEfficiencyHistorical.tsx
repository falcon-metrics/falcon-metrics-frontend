...
export const TrendChart = ({
  ...
  const config = {
    ...
    "scale-x": {
      ...
      label: {
        text: aggregation
          ? aggregation?.charAt(0)?.toUpperCase() + aggregation?.substring(1)
          : "",
        fontSize: "12px",
...
type FlowEfficiencyHistoricalProps = {
  chartTitle?: string;
...
const useHistoricalData = () => {
  ...
  const url = `/value-stream-management/delivery-governance/flow-efficiency-history?${queryParamString}`;

  const { data, error, isValidating } = useCustomSWR<any>(url, fetcher, {
    revalidateOnFocus: false,
    revalidateOnReconnect: false,
  });

  return {
    data: data?.data?.flowEfficiency ?? [],
    error,
    isLoading: isValidating,
  };
};

export const FlowEfficiencyHistorical = ({
  chartTitle,
  telemetryAction,
  telemetrySource,
  aggregationProps,
}: FlowEfficiencyHistoricalProps) => {
  const classes = useStyles();
  const { data: historicalData, error, isLoading } = useHistoricalData();
  ...
  const aggregateDates = useMemo(() => {
    ...
    const dateStrings = historicalData.map((item) => (item && item[0]) || "");
  ...
  function getFillerData() {
    ...
    if (arr.length !== len && historicalDataFormatted[len - 1] === null) {
      const lastNonNull = historicalDataFormatted
        .filter((x) => x !== null)
        .slice(-1)[0];
  ...
  useEffect(() => {
    if (
      ...
      sendTelemetry(
        DrillDownTelemetryAction.accessFitnessCriteriaDrillDown,
        `${telemetrySource} -> Historical View tab`,
        { page: `${telemetrySource} Historical View` }
      );
  }, [sendTelemetry]);
  ...
  if (error) {
    console.error("error when fetching flow efficiency over time : ", error);
    // TODO: Remove this placeholder and put an actual error screen
    return (
      <Box
        fontSize={20}
        textAlign={"center"}
