...
interface PageProps {
  throughputHistoricalChart?: Array<Array<any>>;
  customConfig?: {
    scaleY?: {
      values: string;
      format: string;
...
function getLabelListFromData(
  throughputHistoricalChart: any
): string[] {
  return throughputHistoricalChart.map(item => {
    return formatStringDate(item[0]);
  });
}


export const ThroughputMaxHistoricalView = ({
  throughputHistoricalChart,
  customConfig,
  telemetryAction,
  telemetrySource,
  aggregationProps
}: PageProps) => {
  const results = (throughputHistoricalChart || [])
  ...
  const labels = getLabelListFromData(throughputHistoricalChart);
  ...
  useEffect(() => {
    if (telemetryAction === DrillDownTelemetryAction.accessFitnessCriteriaDrillDown)
      sendTelemetry(DrillDownTelemetryAction.accessFitnessCriteriaDrillDown,
        `${telemetrySource} -> Delivery Rate Predictability (Historical) tab`, { page: "Delivery Rate Predictability" });
  }, [sendTelemetry]);
