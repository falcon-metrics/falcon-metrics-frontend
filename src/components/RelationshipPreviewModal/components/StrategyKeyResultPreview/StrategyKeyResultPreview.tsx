...
const fetchStrategyKeyResult = (url: string) => {
    return fetch.get(`${url}`).then(response => response.data as Strategy);
};

const StrategyKeyResultPreview = (props: PreviewComponentProps) => {
    const { data: response, isValidating } = useSWR(
        `/strategies/keyResult/${props.entityId}`,
        fetchStrategyKeyResult,
...
export default StrategyKeyResultPreview;