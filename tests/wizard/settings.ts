...
// TODO: add spec.ts i.e settings.spec.ts on the filename to be considered as playwright test.
...
test.describe('Set Date Boundaries and Conditions', () => {
    test('should have exclude items completed before date', async ({ page }) => {
        ...
        const excludeDatepicker = await page.getAttribute('input[name="excludeDate"]', 'value');
        console.log(excludeDatepicker);
        expect(excludeDatepicker).not.toBeNull();
    });
});
